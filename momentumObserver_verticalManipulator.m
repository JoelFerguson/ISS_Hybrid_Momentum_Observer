%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Title: Hybrid momentum observer for mechanical systems
% Description: Simulation files for the paper 'Input-to-State Stable hybrid 
% momentum observer for mechanical systems',
% submitted to XXX
...
sim.t_end = 2;      % simulation length

% Observer parameters
obs.kappa = 2.0;

% Control input
ctrl.u = @(t) [8*sin(t); 4*cos(3*t)];
...
if sim.disturbance
    sys.delta_p0 = @(t) [0.5*sin(10*t); 0.5*cos(20*t)];
else
    sys.delta_p0 = @(t) [0; 0];
end
...
sim.ode = @(t,x) [sys.dx(t,x(1:2),x(3:4),sim.u(t));
                obs.dxp(x(1:2),obs.ph(x(1:2),x(5:6),x(7)),sim.u(t),sim.uo(t),x(7));
                obs.dphi];
